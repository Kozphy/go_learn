...
func init() {
	...
	// so that learn_bbgo knows what struct to be used to unmarshal the configs (YAML or JSON)
	// Note: built-in strategies need to imported manually in the learn_bbgo cmd package.
...
type Strategy struct {
	...
	learn_bbgo.OrderExecutor

	// if Symbol string field is defined, learn_bbgo will know it's a symbol-based strategy
	...
	*learn_bbgo.MarketDataStore

	// StandardIndicatorSet contains the standard indicators of a market (symbol)
	// This field will be injected automatically since we defined the Symbol field.
	*learn_bbgo.StandardIndicatorSet
...
func (s *Strategy) notify(format string, args ...interface{}) {
	if channel, ok := learn_bbgo.Notification.RouteSymbol(s.Symbol); ok {
		learn_bbgo.NotifyTo(channel, format, args...)
	} else {
		learn_bbgo.Notify(format, args...)
	}
}
